<template>
  <div class="container overflow-hidden">
    <div class="border-cus-4 min-h-[85dvh] min-w-full">
      <div class="p-3 w-full flex justify-between items-center">
        <div
          class="flex flex-col xs:flex-row gap-2 xs:gap-6 xs:items-center w-full sm:w-auto"
        >
        ...
        <div class="gap-2 items-center flex text-secondary">
          <div
            @click="layout = 'grid'"
            :class="{
              'border border-solid border-[#282828] rounded-full text-primary':
                layout === 'grid',
            }"
          >
            <icons-sort
              class="transition-all duration-200 ease cursor-pointer hover:text-primary"
            />
          </div>
          <div
            @click="layout = 'list'"
        ...
        >
          <template #column-product="{ item }">
            <div class="flex px-5 gap-5 items-center min-w-75">
              <div
                class="h-12 min-w-12 max-w-12 lg:h-16 lg:min-w-16 lg:max-w-16 rounded-xl overflow-hidden"
              >
                <img
              ...
              <div class="flex flex-col justify-center relative">
                ...
                <p
                  class="lg:group-hover/setting:hidden text-sm text-secondary line-clamp-2 opacity-80"
                >
                  {{ (item as Data).description }}
                </p>
                <div
                ...
                >
                  ...
                  <button
                    class="setting-button"
                    @click.stop="toggleDeleteModal((item as Data).id)"
                  >
                    <icons-trash />
                    <p class="">Delete</p>
                  </button>
          ...
          <template #column-sales="{ item }">
            <div class="flex gap-2 items-center">
              <span class="text-primary"
                >${{ (item as Data).sales.toLocaleString("en-US") }}</span
              >
      ...
      >
        <ProductsDraftCard
          ...
          :image="item.image"
          :price="item.price"
          @edit="toggleEditModal(item.id)"
          @delete="toggleDeleteModal(item.id)"
  ...
  <modal-edit
    :isOpen="selectedEditId ? true : false"
    @close="toggleEditModal(null)"
    @save="saveEditModal"
  >
    <form
      class="flex gap-x-4 gap-y-4 flex-wrap max-h-85 sm:max-h-none overflow-y-auto sm:overflow-y-visible"
      v-if="editProduct"
    >
      <div class="flex gap-4 flex-col sm:flex-row w-full">
        <div class="flex flex-col gap-4 w-full sm:w-[calc(50%-8px)]">
          <div class="flex flex-col gap-1 xs:gap-2 w-full">
            ...
            <input
              type="text"
              class="!border border-black rounded-xl p-2 text-sm xs:text-xl h-12"
          ...
          <div class="flex flex-col gap-1 xs:gap-2 w-full">
            ...
            <select-dropdown
              :data="typeList"
              v-model:selected-option="editProduct.description"
        ...
        <div class="flex flex-col gap-2 w-full sm:w-[calc(50%-8px)]">
          ...
          <upload-image
            v-model:preview="editProduct.image"
            image-id="preview"
  ...
  <modal-delete
    :isOpen="selectedDeleteId ? true : false"
...
const columns = [
  {
    slot: "product",
    label: "Product",
    headerClass: "text-left px-5 min-w-70 w-160",
  },
  ...
  { slot: "sales", label: "Sales", headerClass: "text-left min-w-40 w-65.5" },
  {
    slot: "rating",
    label: "Rating",
    headerClass: "text-left min-w-23 w-49.5",
    cellClass:
      "rounded-r-2xl flex h-24 gap-9 justify-between lg:table-cell items-center",
  },
  {
    slot: "views",
    label: "Views",
    headerClass: "text-left hidden xl:table-cell w-24",
    cellClass: "rounded-r-2xl hidden xl:table-cell",
  },
];
